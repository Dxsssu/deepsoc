...
本阶段我们计划在现有多Agent执行链路基础上，引入 MCP（Model Context Protocol）协议，作为外部工具统一接入层。这样可以在尽量不改动现有核心数据模型的前提下，为系统增加更标准化的工具发现、工具调用和结果返回能力。

当前的设计思路是复用现有 `Command` 执行链路，在执行层增加对 MCP 类型命令的支持，使 `Executor` 除了调用 SOAR 剧本和人工命令外，还可以调用标准 MCP 工具。这样既能保持 `Event -> Task -> Action -> Command -> Execution` 主流程稳定，也为后续接入威胁情报、资产查询、日志检索、工单通知等能力预留统一扩展接口。

从阶段性进展看，我们已经完成了相关技术路线的梳理，并明确了“尽量少改模型、优先复用现有执行框架”的实现原则。下一步将围绕 MCP 工具配置、命令执行适配、结果回写和安全控制策略逐步落地。
