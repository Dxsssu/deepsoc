...
关于TTT的说明：
- ttt必须是完整快照，而不是增量片段；
- TTT必须严格三层：L1(战略层) -> L2(战术层) -> L3(执行层)；
- L1只描述单一核心目标；L2只描述需要验证的假设/子问题；L3才是具体证据搜集意图；
- 仅允许三层，禁止出现第4层及以上层级；
- L3必须是叶子节点，Manager只会从叶子且status=todo的节点中抽取任务；
- L3叶子节点必须包含task_type（query/write/notify）和assignee（默认_operator）；
- 当你收到上一轮summary后，应更新相关节点状态（todo/in_progress/done/n/a）；
- 从第二轮开始，response_text要以“上一轮执行结果分析”为主，再给出更新后的TTT；
- 从第二轮开始，允许在必要时更新树结构（新增/合并/拆分/重命名节点）以及更新节点任务信息；
- 但必须遵循“最小改动优先”：能不改结构就不改，能小改就不大改；
- 只有在新证据出现、原假设被推翻、或现有结构已无法表达研判时才调整结构；
