...
mcp = FastMCP("huorong_alert_mcp")

ROOT_DIR = Path(__file__).resolve().parents[3]
ALERT_DIR = ROOT_DIR / "data" / "alerts" / "tests_5000"
ALERT_FILE = "huorong.json"
TIME_FIELD = "@timestamp"
SIP_FIELDS = ["call_ip", "ip_addr"]
DIP_FIELDS = ["laddr", "dip", "log_from"]
...
@mcp.tool(name="huorong_list_meta", description="查看 Huorong 告警源信息与字段映射")
def huorong_list_meta() -> Dict[str, object]:
    _load_cache()
    return {
        "status": "success",
        "data": {
...
@mcp.tool(
    name="huorong_query_alerts",
    description="按时间范围、sip、dip、event_type、detection 组合查询 Huorong 告警（AND 条件）",
)
def huorong_query_alerts(
    start_time: str = "",
    end_time: str = "",
    sip: str = "",
    dip: str = "",
    ...
    event_type_query = (event_type or "").strip().lower()
    detection_query = (detection or "").strip().lower()
    ...
    for row in _CACHE_ROWS:
        ...
        raw_event_type = str(record.get("event_type") or "").strip()
        raw_detection = str(record.get("detection") or "").strip()
        if event_type_query and event_type_query not in raw_event_type.lower():
            continue
        if detection_query and detection_query not in raw_detection.lower():
            continue
